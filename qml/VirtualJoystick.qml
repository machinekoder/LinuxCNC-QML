...
Rectangle {
    ...
    readonly property double xVelocity: {
        Math.floor(((control.x + maxControlX()) / Math.abs(maxControlX()) * maxVelocity) / stepSize) * stepSize
    }
    readonly property double yVelocity: {
        Math.floor(((control.y + maxControlY()) / Math.abs(maxControlY()) * maxVelocity) / stepSize) * stepSize
    }
